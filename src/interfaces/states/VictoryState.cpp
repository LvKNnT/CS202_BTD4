...
VictoryState::VictoryState() : State(377, 511, Game::Instance().getTextureManager().getTexture("Table")) {
    ...
    std::shared_ptr<PanelElement> titleText = std::make_shared<BackHome>(Game::Instance().getTextureManager().getTexture("Victory"), 0, 204, 400, (Vector2) {tablePos.x + 55, tablePos.y - 50});
...
void VictoryState::update(Event::Type event) {
}
