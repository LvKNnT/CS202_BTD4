...
MOABAssassinSkill::MOABAssassinSkill()
    : Skill(30.0f, "MOAB Assassin") {
    // Instant cooldown
    timer = cooldown;
}

std::unique_ptr<Skill> MOABAssassinSkill::clone() const {
    return std::make_unique<MOABAssassinSkill>(*this);
}

void MOABAssassinSkill::loadTexture() {
    // Load the texture for the MOAB Assassin skill
    Game::Instance().getTextureManager().loadTexture(name, "../assets/tower/Bomb_Shooter/MOABAssassinUpgradeIcon.png");
}

void MOABAssassinSkill::update() {
...
bool MOABAssassinSkill::canActivateSkill() const {
    // Check if the skill is ready to be activated
    return timer >= cooldown;
}

float MOABAssassinSkill::getCooldown() const {
    return cooldown;
}

void MOABAssassinSkill::activateSkill(std::shared_ptr<Tower> tower, std::vector<std::shared_ptr<Enemy>>& enemies, std::vector<std::shared_ptr<Tower>>& towers, BulletManager& bulletManager, ResourceManager& resourceManager) {
    if (!tower) return;
    ...
    if (targetEnemy) {
        ...
        bulletManager.spawnBullet(BulletType::MOABAssassin, position, 
            ...
            rotation,
            750 + SkillFriendAccess::getAttackBuff(*tower).damage,
