#include "ReverseDuneSea.h"

ReverseDuneSea::ReverseDuneSea() : pathIdxCount(0) {
    mapType = MapType::ReverseDuneSea;
    MAXPATHS = 2; 
    // path 0
    enemyPath[0].push_back(Point(-50, 518, Point::Type::SpawnEnenmy)); // enemy goes from there
    ...
    enemyPath[0].push_back(Point(-100, 37, Point::Type::Exit)); // player gets damage here
    enterEffects.push_back(Animation("movingTriangle", (Vector2) {70, 500}, 90.0f, 64, 40, 8, 0.25f));
    enterEffects.back().start();
    
    // path 1
    enemyPath[1].push_back(Point(1050, 571, Point::Type::SpawnEnenmy)); // enemy goes from there
    ...
    enemyPath[1].push_back(Point(1100, 8, Point::Type::Exit)); // player gets damage here
    enterEffects.push_back(Animation("movingTriangle", (Vector2) {930, 555}, -90.0f, 64, 40, 8, 0.25f));
    enterEffects.back().start();
}

std::unique_ptr<Map> ReverseDuneSea::clone() const {
    return std::make_unique<ReverseDuneSea>(*this);
}

Point::Type ReverseDuneSea::getPointType(Vector2 position) const {
...
void ReverseDuneSea::loadTexture() {
...
std::pair<Vector2, int> ReverseDuneSea::getPositionAndPathIdx(BloonType type) {
