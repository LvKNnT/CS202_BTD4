#include "AttackOnBloon.h"

AttackOnBloon::AttackOnBloon(){
    ...
    enemyPath[0].push_back(Point(1100, 578, Point::Type::Exit)); // player gets damage here

    enterEffects.push_back(Animation("movingTriangle", (Vector2) {355, 70}, 180.0f, 64, 40, 8, 0.25f));
    enterEffects.back().start();
}

std::unique_ptr<Map> AttackOnBloon::clone() const {
    return std::make_unique<AttackOnBloon>(*this);
}

Point::Type AttackOnBloon::getPointType(Vector2 position) const {
...
void AttackOnBloon::loadTexture() {
...
std::pair<Vector2, int> AttackOnBloon::getPositionAndPathIdx(BloonType type) {
