...
SniperMonkey::SniperMonkey(const SniperMonkey& other)
: Tower(other) {
}

std::unique_ptr<Tower> SniperMonkey::clone() const {
    return std::make_unique<SniperMonkey>(*this);
}

void SniperMonkey::loadTexture() {
    // Load the texture for the Sniper Monkey tower
    Game::Instance().getTextureManager().loadTexture(tag, "../assets/tower/Sniper_Monkey/Sniper_Monkey_Flash.png");
    ...
    size = {60.0f, 100.0f}; // Set the size of the Sniper Monkey tower
...
void SniperMonkey::update() {
    // Sniper Monkey has no special update.
...
void SniperMonkey::setRotation(float rotation) {
    this->rotation = rotation;
}

void SniperMonkey::draw() const {
    // Draw the Sniper Monkey tower using raylib functions
    ...
    DrawTexturePro(Game::Instance().getTextureManager().getTexture(tag), 
                   ...
                   rotation,
                   WHITE); // Draw the Sniper Monkey texture with the specified position and rotation
...
void SniperMonkey::drawRange() const {
    // Use dummy range
    float range = 40.0f;
    DrawCircleV(position, range * attackBuff.rangeRatio + attackBuff.range, Fade(GRAY, 0.5f)); // Draw the attack range
}

void SniperMonkey::drawPut() const {
    // Draw the range of attacks
    float range = 40.0f;
    if(isActive()) {
        DrawCircleV(position, range * attackBuff.rangeRatio + attackBuff.range, Fade(GRAY, 0.5f)); 
    }
    else DrawCircleV(position, range * attackBuff.rangeRatio + attackBuff.range, Fade(RED, 0.5f)); 
}

void SniperMonkey::setModifies(const TowerModifies& modifies) {
    // Set the tower modifies for the Sniper Monkey
...
LogicInfo SniperMonkey::getInfo() {
...
bool SniperMonkey::isPlaceable(Point::Type pointType) const {
    ...
    return pointType == Point::Type::None || pointType == Point::Type::BetweenWaterAndLane; // Sniper Monkey can only be placed on land
}
...
bool SniperMonkey::isActive() const {
    return isActiveFlag;
}

void SniperMonkey::setActive(bool active) {
    isActiveFlag = active;
}