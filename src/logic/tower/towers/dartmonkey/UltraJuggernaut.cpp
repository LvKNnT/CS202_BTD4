...
void UltraJuggernaut::update(std::vector<std::unique_ptr<Attack>>& attacks, AttackBuff& attackBuff, std::unique_ptr<AttackPattern>& attackPattern) {
    for (auto& attack : attacks) {
        ...
        if (attack->getTag() == "JuggernautAttack") {
            attacks.back() = std::make_unique<UltraJuggernautAttack>(128.0f, 1.0f, attacks.back()->getPosition(), attacks.back()->getTowerId(), 5, 600, 210, 10.0f, BulletProperties{true, true, true, true, false, true}); 
        }
    }
}
