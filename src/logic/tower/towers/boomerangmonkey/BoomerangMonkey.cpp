...
BoomerangMonkey::BoomerangMonkey(Vector2 position)
    : Tower(position, {0.0f, 0.0f}, 0.0f, TowerType::BoomerangMonkey, 315) {
    /**
     * cost = 315
     */
...
BoomerangMonkey::BoomerangMonkey(const BoomerangMonkey& other)
: Tower(other) {
}

std::unique_ptr<Tower> BoomerangMonkey::clone() const {
    return std::make_unique<BoomerangMonkey>(*this);
}

void BoomerangMonkey::loadTexture() {
    // Load the texture for the Boomerang Monkey tower
    Game::Instance().getTextureManager().loadTexture(tag, "../assets/tower/Boomerang_Monkey/BoomerangMonkey.png");
    ...
    size = {60.0f, 60.0f}; // Set the size of the Boomerang Monkey tower
...
void BoomerangMonkey::update() {
    // Boomerang Monkey has no special update.
...
void BoomerangMonkey::setRotation(float rotation) {
    this->rotation = rotation;
}

void BoomerangMonkey::draw() const {
    // Draw the Boomerang Monkey tower using raylib functions
    ...
    DrawTexturePro(Game::Instance().getTextureManager().getTexture(tag), 
                   ...
                   rotation,
                   WHITE); // Draw the Boomerang Monkey texture with the specified position and rotation
...
void BoomerangMonkey::setModifies(const TowerModifies& modifies) {
    // Set the tower modifies for the Boomerang Monkey
...
LogicInfo BoomerangMonkey::getInfo() {
...
Rectangle BoomerangMonkey::getBoundingBox() const {
...
void BoomerangMonkey::setActive(bool active) {
    isActiveFlag = active;
}