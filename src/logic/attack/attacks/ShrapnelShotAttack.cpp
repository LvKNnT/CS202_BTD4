#include "ShrapnelShotAttack.h"

#include "../../bullet/bullets/ShrapnelShotBullet.h"

#include "raymath.h"

ShrapnelShotAttack::ShrapnelShotAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "ShrapnelShotAttack"; 
}

std::unique_ptr<Attack> ShrapnelShotAttack::clone() const {
    // Create a new ShrapnelShotAttack instance with the same properties
    return std::make_unique<ShrapnelShotAttack>(*this);
}

bool ShrapnelShotAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void ShrapnelShotAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void ShrapnelShotAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
    // Update the attack logic, e.g., spawn a shrapnel shot if the cooldown is over
    if (timer <= 0.0f) {
        ...
        attackPattern->execute(bulletManager, BulletType::ShrapnelShot, position, 
