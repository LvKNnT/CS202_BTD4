...
std::unique_ptr<Attack> BombAttack::clone() const {
    // Create a new BombAttack instance with the same properties
    return std::make_unique<BombAttack>(*this);
}

bool BombAttack::isInRange(const Rectangle& rec, const float rotation, bool isCamo, AttackBuff& attackBuff) const {
...
void BombAttack::update(BulletManager& bulletManager, const Vector2& targetPosition, AttackBuff& attackBuff, AttackPattern& attackPattern) {
