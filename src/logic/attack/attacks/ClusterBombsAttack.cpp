...
ClusterBombsAttack::ClusterBombsAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "ClusterBombsAttack"; 
}

std::unique_ptr<Attack> ClusterBombsAttack::clone() const {
    // Create a new ClusterBombsAttack instance with the same properties
    return std::make_unique<ClusterBombsAttack>(*this);
}

bool ClusterBombsAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void ClusterBombsAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void ClusterBombsAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
