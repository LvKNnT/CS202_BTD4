#include "RecursiveClusterAttack.h"
...
RecursiveClusterAttack::RecursiveClusterAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "RecursiveClusterAttack"; 
}

std::unique_ptr<Attack> RecursiveClusterAttack::clone() const {
    // Create a new RecursiveClusterAttack instance with the same properties
    return std::make_unique<RecursiveClusterAttack>(*this);
}

bool RecursiveClusterAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void RecursiveClusterAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void RecursiveClusterAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
