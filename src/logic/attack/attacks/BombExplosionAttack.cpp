#include "BombExplosionAttack.h"

#include "../../bullet/bullets/BombExplosion.h"

#include <cmath>

BombExplosionAttack::BombExplosionAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "BombExplosionAttack"; 
}

std::unique_ptr<Attack> BombExplosionAttack::clone() const {
    // Create a new BombExplosionAttack instance with the same properties
    return std::make_unique<BombExplosionAttack>(*this);
}

bool BombExplosionAttack::isInRange(const Rectangle& rec, const float rotation, bool isCamo, AttackBuff& attackBuff) const {
...
void BombExplosionAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void BombExplosionAttack::update(BulletManager& bulletManager, const Vector2& targetPosition, AttackBuff& attackBuff, AttackPattern& attackPattern) {
    // Update the attack logic, e.g., spawn a bomb explosion if the cooldown is over
    if (timer <= 0.0f) {
        ...
        attackPattern.execute(bulletManager, BulletType::BombExplosion, position, {100.0f, 100.0f}, angle, 
