#include "UltraJuggernautAttack.h"

#include "../../bullet/bullets/UltraJuggernautBullet.h"
...
std::unique_ptr<Attack> UltraJuggernautAttack::clone() const {
    // Create a new UltraJuggernautAttack instance with the same properties
    return std::make_unique<UltraJuggernautAttack>(*this);
}

bool UltraJuggernautAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void UltraJuggernautAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
    ...
    if (timer <= 0.0f) {
        ...
        attackPattern->execute(bulletManager, BulletType::UltraJuggernaut, position, 
            Vector2Add({60.0f, 60.0f}, attackBuff.size),
            angle, 
