...
#include "../../bullet/bullets/Sub.h"

#include <cmath>
#include "raymath.h"

SubAttack::SubAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "SubAttack"; 
}

std::unique_ptr<Attack> SubAttack::clone() const {
    // Create a new SubAttack instance with the same properties
    return std::make_unique<SubAttack>(*this);
}

bool SubAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void SubAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void SubAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
    // Update the attack logic, e.g., spawn a Sub if the cooldown is over
    if (timer <= 0.0f) {
        ...
        if(properties.canTrace) {
            ...
            // std::cerr << "Sub bullet is tracing enemy with ID: " << enemy->getId() << std::endl;
        }
        
        attackPattern->execute(bulletManager, BulletType::Sub, position, 
