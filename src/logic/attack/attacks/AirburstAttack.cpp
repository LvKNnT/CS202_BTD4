#include "AirburstAttack.h"

#include "../../bullet/bullets/Airburst.h"

#include <cmath>
#include "raymath.h"

AirburstAttack::AirburstAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "AirburstAttack"; 
}

std::unique_ptr<Attack> AirburstAttack::clone() const {
    // Create a new AirburstAttack instance with the same properties
    return std::make_unique<AirburstAttack>(*this);
}

bool AirburstAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void AirburstAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void AirburstAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
    // Update the attack logic, e.g., spawn a Airburst if the cooldown is over
    if (timer <= 0.0f) {
        ...
        if(properties.canTrace) {
            ...
            // std::cerr << "Airburst bullet is tracing enemy with ID: " << enemy->getId() << std::endl;
        }
        
        attackPattern->execute(bulletManager, BulletType::Airburst, position, 
