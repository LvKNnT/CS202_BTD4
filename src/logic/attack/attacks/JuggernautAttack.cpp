...
bool JuggernautAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void JuggernautAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void JuggernautAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
