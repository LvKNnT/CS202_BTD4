#include "ReallyBigBombsAttack.h"

#include "../../bullet/bullets/ReallyBigBombsBullet.h"

#include <cmath>
#include "raymath.h"

ReallyBigBombsAttack::ReallyBigBombsAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "ReallyBigBombsAttack"; 
}

std::unique_ptr<Attack> ReallyBigBombsAttack::clone() const {
    // Create a new ReallyBigBombsAttack instance with the same properties
    return std::make_unique<ReallyBigBombsAttack>(*this);
}

bool ReallyBigBombsAttack::isInRange(const Rectangle& rec, const float rotation, bool isCamo, AttackBuff& attackBuff) const {
...
void ReallyBigBombsAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void ReallyBigBombsAttack::update(BulletManager& bulletManager, const Vector2& targetPosition, AttackBuff& attackBuff, AttackPattern& attackPattern) {
    // Update the attack logic, e.g., spawn a really big bomb bullet if the cooldown is over
    if (timer <= 0.0f) {
        ...
        timer = cooldown * attackBuff.cooldownRatio; // Reset the timer with cooldown ratio
    }
}