#include "ArmourPiercingAttack.h"

#include "../../bullet/bullets/ArmourPiercing.h"

#include <cmath>
#include "raymath.h"

ArmourPiercingAttack::ArmourPiercingAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "ArmourPiercingAttack"; 
}

std::unique_ptr<Attack> ArmourPiercingAttack::clone() const {
    // Create a new ArmourPiercingAttack instance with the same properties
    return std::make_unique<ArmourPiercingAttack>(*this);
}

bool ArmourPiercingAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void ArmourPiercingAttack::update() {
    if (timer > 0.0f) {
        timer -= GetFrameTime(); 
    }
}

void ArmourPiercingAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
    // Update the attack logic, e.g., spawn a ArmourPiercing if the cooldown is over
    if (timer <= 0.0f) {
        ...
        if(properties.canTrace) {
            ...
            // std::cerr << "ArmourPiercing bullet is tracing enemy with ID: " << enemy->getId() << std::endl;
        }
        
        attackPattern->execute(bulletManager, BulletType::ArmourPiercing, position, 
