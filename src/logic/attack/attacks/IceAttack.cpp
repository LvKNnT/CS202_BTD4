...
IceAttack::IceAttack(float range, float cooldown, Vector2 position, int towerId, int damage, int speed, int pierce, float lifeSpan, BulletProperties properties, BloonDebuff normalDebuff, BloonDebuff moabDebuff)
    : Attack(range, cooldown, position, towerId, damage, speed, pierce, lifeSpan, properties, normalDebuff, moabDebuff) {
    // Constructor implementation can be extended if needed
    tag = "IceAttack"; 
}

std::unique_ptr<Attack> IceAttack::clone() const {
    // Create a new IceAttack instance with the same properties
    return std::make_unique<IceAttack>(*this);
}

bool IceAttack::isInRange(const Rectangle& rec, const float rotation, AttackBuff& attackBuff, const Enemy &enemy) const {
...
void IceAttack::update(BulletManager& bulletManager, std::shared_ptr<Enemy>& enemy, AttackBuff& attackBuff) {
    ...
    if (timer <= 0.0f) {
        ...
        attackPattern->execute(bulletManager, BulletType::Ice, position, 
