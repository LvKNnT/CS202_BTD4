...
std::unique_ptr<Bullet> Sub::clone() const {
    return std::make_unique<Sub>(*this);
}

void Sub::loadTexture() {
    // Load the texture for the Sub bullet
    Game::Instance().getTextureManager().loadTexture(tag, "../assets/bullet/AirBurstDart.png");
...
void Sub::init(Vector2 position, Vector2 size, float rotation, int damage, int speed, int pierce, float lifeSpan, BulletProperties& properties, BloonDebuff& normalDebuff, BloonDebuff& moabDebuff, AttackBuff& attackBuff, int towerId) {
...
void Sub::update(std::vector<std::shared_ptr<Enemy>>& enemyList) {
    // no special update
}

bool Sub::hit(int damage) {
    pierce -= damage;
...
void Sub::draw() const {
    ...
    // DrawCircleV(position, 10, RED); // Example drawing a red circle for the Sub
    ...
    DrawTexturePro(Game::Instance().getTextureManager().getTexture(tag), 
                   ...
                   rotation,
                   WHITE); // Draw the Sub texture with the specified position and rotation
}

int Sub::die() {
    // Logic for when the Sub bullet reaches the end of its life
    // For example, you might want to remove it from the game or trigger an event
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "Sub bullet reached the end of its life!" << std::endl;
    flog.close();

    return -1;
}

std::vector<std::unique_ptr<Bullet>> Sub::getChild() {
    // Sub bullet does not spawn any child bullets, so return an empty vector
    return {};
}

Rectangle Sub::getBoundingBox() const {
    return {position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y};
}

bool Sub::isActive() const {
    // Logic to determine if the Sub bullet is still active
    // For example, you might check if it has reached its target or if it has been destroyed
    return true; // Assuming the Sub is always active for this example
}

void Sub::setActive(bool active) {
    // Logic to set the active state of the Sub bullet
    // For example, you might want to deactivate it when it hits a target or reaches its end
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "Sub bullet active state set to: " << (active ? "true" : "false") << std::endl;
    flog.close();
}

void Sub::setRotation(float rotation) {
    // Set the rotation of the Sub bullet
