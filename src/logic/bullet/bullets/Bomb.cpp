...
Bomb::Bomb()
    : Bullet(BulletType::Bomb) {
    tag = "Bomb";
}

std::unique_ptr<Bullet> Bomb::clone() const {
    return std::make_unique<Bomb>(*this);
}

void Bomb::loadTexture() {
    // Load the texture for the Bomb bullet
...
void Bomb::setActive(bool active) {
    ...
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "Bomb bullet active state set to: " << (active ? "true" : "false") << std::endl;
    flog.close();
}

void Bomb::setRotation(float rotation) {
