...
PineApple::PineApple()
    : Bullet(BulletType::PineApple) {
    tag = "PineApple";
}

std::unique_ptr<Bullet> PineApple::clone() const {
    return std::make_unique<PineApple>(*this);
}

void PineApple::loadTexture() {
    // Load the texture for the PineApple bullet
    Game::Instance().getTextureManager().loadTexture(tag, "../assets/bullet/PineApple.png");
...
void PineApple::init(Vector2 position, Vector2 size, float rotation, int damage, int speed, int pierce, float lifeSpan, BulletProperties& properties, BloonDebuff& normalDebuff, BloonDebuff& moabDebuff, AttackBuff& attackBuff, int towerId) {
...
void PineApple::update(std::vector<std::shared_ptr<Enemy>>& enemyList) {
    // no special update
}

bool PineApple::hit(int damage) {
    // only 1 pierce
    
    return true;
}

int PineApple::getDamage(BloonType type, bool isCamo) {
    // the projectile deals no dmg, only the explosion does
    return 0;
}

void PineApple::draw() const {
    // Check if the bullet is active before drawing
    if(!isActiveFlag) {
        return; 
    }

    //DrawCircleV(position, 10, RED); // Example drawing a red circle for the PineApple
...
int PineApple::die() {
    ...
    flog << "PineApple bullet reached the end of its life!" << std::endl;
    flog.close();

    return -1;
}

std::vector<std::unique_ptr<Bullet>> PineApple::getChild() {
...
Rectangle PineApple::getBoundingBox() const {
    return {position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y};
}

bool PineApple::isActive() const {
    // Logic to determine if the PineApple bullet is still active
    // For example, you might check if it has reached its target or if it has been destroyed
    return true; // Assuming the PineApple is always active for this example
}

void PineApple::setActive(bool active) {
    // Logic to set the active state of the PineApple bullet
    // For example, you might want to deactivate it when it hits a target or reaches its end
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "PineApple bullet active state set to: " << (active ? "true" : "false") << std::endl;
    flog.close();
}

void PineApple::setRotation(float rotation) {
    // Set the rotation of the PineApple bullet
