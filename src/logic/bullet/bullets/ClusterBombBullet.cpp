...
ClusterBombBullet::ClusterBombBullet()
    : Bullet(BulletType::ClusterBomb) {
    tag = "ClusterBombBullet";
}

std::unique_ptr<Bullet> ClusterBombBullet::clone() const {
    return std::make_unique<ClusterBombBullet>(*this);
}

void ClusterBombBullet::loadTexture() {
...
void ClusterBombBullet::init(Vector2 position, Vector2 size, float rotation, int damage, int speed, int pierce, float lifeSpan, BulletProperties& properties, BloonDebuff& normalDebuff, BloonDebuff& moabDebuff, AttackBuff& attackBuff, int towerId) {
...
bool ClusterBombBullet::hit(int damage) {
    // only 1 pierce

    return true; 
}

void ClusterBombBullet::draw() const {
...
Rectangle ClusterBombBullet::getBoundingBox() const {
    return {position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y};
}

bool ClusterBombBullet::isActive() const {
    return isActiveFlag;
}

void ClusterBombBullet::setActive(bool active) {
    isActiveFlag = active;
}

int ClusterBombBullet::die() {
    isActiveFlag = false; // Mark the bullet as inactive

    return 0; // Return 0 to indicate the bullet has died
}

std::vector<std::unique_ptr<Bullet>> ClusterBombBullet::getChild() {
    ...
    for(int i = 0; i < 8; ++i) {
        ...
        children[i]->init(childrenPosition, 
                          ...
                          1 + attackBuff.damage, 
                          speed, 
                          8 * attackBuff.pierceRatio + attackBuff.pierce, 
...
void ClusterBombBullet::setRotation(float rotation) {
