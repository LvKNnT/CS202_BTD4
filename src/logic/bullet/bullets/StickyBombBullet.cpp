#include "StickyBombBullet.h"
#include "BombExplosion.h"
#include "../../../core/Game.h"

#include <fstream>

StickyBombBullet::StickyBombBullet()
    : Bullet(BulletType::StickyBomb) {
    tag = "StickyBomb";
}

std::unique_ptr<Bullet> StickyBombBullet::clone() const {
    return std::make_unique<StickyBombBullet>(*this);
}

void StickyBombBullet::loadTexture() {
    // Load the texture for the StickyBombBullet bullet
    Game::Instance().getTextureManager().loadTexture(tag, "../assets/bullet/StickyBomb.png");
...
void StickyBombBullet::init(Vector2 position, Vector2 size, float rotation, int damage, int speed, int pierce, float lifeSpan, BulletProperties& properties, BloonDebuff& normalDebuff, BloonDebuff& moabDebuff, AttackBuff& attackBuff, int towerId) {
...
void StickyBombBullet::update(std::vector<std::shared_ptr<Enemy>>& enemyList) {
    // no special update
}

bool StickyBombBullet::hit(int damage) {
    // only 1 pierce
    return true;
}

int StickyBombBullet::getDamage(BloonType type, bool isCamo) {
    // the projectile deals no dmg, only the explosion does
    return 0;
}

void StickyBombBullet::draw() const {
    // Check if the bullet is active before drawing
    if(!isActiveFlag) {
        return; 
    }

    //DrawCircleV(position, 10, RED); // Example drawing a red circle for the StickyBombBullet
...
int StickyBombBullet::die() {
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "StickyBombBullet bullet reached the end of its life!" << std::endl;
    flog.close();

    return -1;
}

std::vector<std::unique_ptr<Bullet>> StickyBombBullet::getChild() {
    ...
    auto childProperties = properties;
    childProperties.isOnlyFollowing = false;
...
Rectangle StickyBombBullet::getBoundingBox() const {
    return {position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y};
}

bool StickyBombBullet::isActive() const {
...
void StickyBombBullet::setActive(bool active) {
    ...
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "StickyBombBullet active state set to: " << (active ? "true" : "false") << std::endl;
    flog.close();
}

void StickyBombBullet::setRotation(float rotation) {
