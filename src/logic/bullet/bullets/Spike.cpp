...
Spike::Spike()
    : Bullet(BulletType::Spike) {
    tag = "Spike";
}

std::unique_ptr<Bullet> Spike::clone() const {
    return std::make_unique<Spike>(*this);
}

void Spike::loadTexture() {
    // Load the texture for the Spike bullet
    Game::Instance().getTextureManager().loadTexture(tag, "../assets/bullet/Spike.png");
...
void Spike::init(Vector2 position, Vector2 size, float rotation, int damage, int speed, int pierce, float lifeSpan, BulletProperties& properties, BloonDebuff& normalDebuff, BloonDebuff& moabDebuff, AttackBuff& attackBuff, int towerId) {
...
void Spike::update(std::vector<std::shared_ptr<Enemy>>& enemyList) {
    // no special update
}

bool Spike::hit(int damage) {
    pierce -= damage;
...
void Spike::draw() const {
    ...
    // DrawCircleV(position, 10, RED); // Example drawing a red circle for the Spike
    ...
    DrawTexturePro(Game::Instance().getTextureManager().getTexture(tag), 
                   ...
                   rotation,
                   WHITE); // Draw the Spike texture with the specified position and rotation
}

int Spike::die() {
    // Logic for when the Spike bullet reaches the end of its life
    // For example, you might want to remove it from the game or trigger an event
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "Spike bullet reached the end of its life!" << std::endl;
    flog.close();

    return -1;
}

std::vector<std::unique_ptr<Bullet>> Spike::getChild() {
    // Spike bullet does not spawn any child bullets, so return an empty vector
    return {};
}

Rectangle Spike::getBoundingBox() const {
    return {position.x - size.x / 2.0f, position.y - size.y / 2.0f, size.x, size.y};
}

bool Spike::isActive() const {
    // Logic to determine if the Spike bullet is still active
    // For example, you might check if it has reached its target or if it has been destroyed
    return true; // Assuming the Spike is always active for this example
}

void Spike::setActive(bool active) {
    // Logic to set the active state of the Spike bullet
    // For example, you might want to deactivate it when it hits a target or reaches its end
    std::fstream flog("../logs/log.txt", std::ios::out | std::ios::app);
    flog << "Spike bullet active state set to: " << (active ? "true" : "false") << std::endl;
    flog.close();
}

void Spike::setRotation(float rotation) {
    // Set the rotation of the Spike bullet
